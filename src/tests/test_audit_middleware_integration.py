...
@pytest.mark.anyio
@pytest.mark.usefixtures("db")
async def test_audit_middleware_authorizes_vps_containers_as_session_tool(
    mocker: MockerFixture,
) -> None:
    """Verify VPS container diagnostics use the session caller allowlist."""

    caller = await McpCallerFactory.save_to_db(
        client_id="session-vps-containers-client",
        client_type="codex",
        workspace=LogWorkspace.SESSION,
